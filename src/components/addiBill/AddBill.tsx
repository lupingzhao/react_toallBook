...
import { Keyboard, Button } from 'zarm'
import styles from '../../App.module.scss'
import { Input, DatePicker } from 'zarm'
import dayjs from '_dayjs@1.10.6@dayjs';

import { addBill } from '../../store/actions/bill'
import { useSelector, useDispatch } from 'react-redux'
import { number } from '_echarts@5.1.2@echarts';
import CcKeyboard from './../cc-keyboard/cc-keyboard';
import '../../components/cc-keyboard/index.scss'
...
const AddBill = (props: Props) => {
  ...
  let confim = () => {
    if (inputdatas) {
      ...
      if (remark) {
        // 当天
        dispatch(addBill(Number(inputdatas), typeId, typename, Number(time), activetab + 1, remark))
      } else dispatch(addBill(Number(inputdatas), typeId, typename, Number(time), activetab + 1))
      // 分发事件
      props.send(false)
  ...
  return (
    <div className={`${styles.addBill}`}>
      <div className='t-a-r p-10 font-s-18' onClick={() => { props.send(false) }}>X</div>
      <div className='flex jcsb p-10 '>
        <div className='flex'>
          <div className={`m-r-15 ${activetab === 0 ? styles.activetab : styles.tab}`} onClick={() => { setActivetab(0), setActiveICON(0), setTypeId(1), setTypename('餐饮') }}>支出</div>
          <div className={`${activetab === 1 ? styles.activetab1 : styles.tab} `} onClick={() => { setActivetab(1), setActiveICON(10), setTypeId(11), setTypename('工资') }}> 收入</div>
        </div>
        <div >
          <div className={`${styles.date1}`} style={{ backgroundColor: ' rgba(0, 0, 0, 0.1) ', borderRadius: '15px' }} onClick={() => {
            setVisible2(true)
          }}>
            ...
            props.list.map((item: any, index: number) => {
              return (
                item.type === '1' ? <div key={index}>
                  <div className={` t-a-c p-10`} style={{ width: 100, }} onClick={() => { setActiveICON(index), setTypeId(item.id), setTypename(item.name) }}>
                    <div>
                      <i className={`iconfont ${icons[index]} ${activeICON === index ? styles.spen : ''}`} style={{ fontSize: 25 }}></i>
                    </div>
                    <div>
                      {item.name}
                    </div>
            ...
            props.list.map((item: any, index: number) => {
              return (
                item.type === '2' ? <div key={index}>
                  <div className={` t-a-c p-10`} style={{ width: 100, }} onClick={() => { setActiveICON(index), setTypeId(item.id), setTypename(item.name) }}>
                    <div>
                      <i className={`iconfont ${icons[index]} ${activeICON === index ? styles.come : ''}`} style={{ fontSize: 25 }}></i>
                    </div>
                    <div>
                      {item.name}
                    </div>
      ...
      <div className='p-10'>
        {
          beizhu ? <span className='p-10' style={{ color: "#007FFF" }} onClick={() => {
            setBeizhu(false)
          }}>添加备注</span> : <div className='border p-10'><Input showLength autoHeight type="text" rows={4} maxLength={50} placeholder="摘要" onChange={(value: any) => {
            setRemark(value);
          }} /></div>
        }
      </div>
